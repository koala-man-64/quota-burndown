...
In the live dashboard, expand **View raw text** beneath a Recent requests row to load its recorded user prompt and response. Only the 25 rows in the current dashboard snapshot are accessible; reload the page if a request has left that snapshot. Text is fetched on demand from local provider transcripts and is never added to the ledger, static HTML, or normal usage exports. The viewer preserves plain text and line breaks and excludes tool payloads and hidden reasoning. It does not reconstruct the full API request or conversation history. Missing sources, ambiguous matches, and read/display limits are reported explicitly; incomplete text is labeled partial. Static fallback pages do not offer the viewer.
